...
/** Object registry (`dobj`) columns excluded from inline-edit picker (system + non-editable UI columns). */
export const DOBJ_REGISTRY_INLINE_EDIT_EXCLUDED: readonly string[] = [
    "sys_id",
    ...DOBJ_READ_ONLY_BASE,
    "dobj_name_display",
    "dobj_description",
...
export const DOBJ_REGISTRY_INLINE_EDIT_EXCLUDED_SET = new Set(
    DOBJ_REGISTRY_INLINE_EDIT_EXCLUDED.map((k) => k.toLowerCase()),
);

export function isDobjRegistryInlineEditExcluded(apiName: string): boolean {
    return DOBJ_REGISTRY_INLINE_EDIT_EXCLUDED_SET.has(String(apiName ?? "").trim().toLowerCase());
}
