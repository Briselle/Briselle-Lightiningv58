...
export const FIELD_ATTRIBUTE_UI_SECTIONS: FieldAttributeUiSection[] = [
    ...
    {
        id: 'dataView',
        title: 'Data View Configuration',
        description:
            'Preferred columns, table visibility, and inline editing for this field (combined with table / preset settings).',
        shellClass: 'rounded-lg border border-teal-200/90 bg-teal-50/30 shadow-sm ring-1 ring-teal-200/35',
        keys: ['preferredInView', 'includeInTableView', 'includeInInlineEdit'],
    },
    {
        id: 'typeSpecific',
...
export function FieldAttributesSectionedPanel({
    ...
    const fieldSpecificSectionIds: readonly string[] = ['typeSpecific', 'dataView'];
    ...
    const pairedTypeAndDataView =
        fieldSpecificRows.some((r) => r.section.id === 'typeSpecific') &&
        fieldSpecificRows.some((r) => r.section.id === 'dataView');
    const fieldSpecificGridClass = pairedTypeAndDataView
    ...
    const renderSection = ({ section, fieldKeys }: (typeof rows)[number]) => (
        <section
            key={section.id}
            className={`${compact ? 'p-2' : 'p-4'} ${section.shellClass} min-w-0`}
    ...
    const inner = (
        <div className={compact ? 'space-y-2' : 'space-y-4'}>
            ...
            {fieldSpecificRows.length > 0 ? (
                <div className={fieldSpecificGridClass}>
