/**
 * Master catalog for custom object field types (object-manager parity, storage-oriented).
...
export interface FieldTypeMasterEntry {
    ...
    /** UI section / optgroup */
    category: string;
    /** Picker label shown in the data type selector */
    label: string;
    /** Short “what is this?” description for the type */
    description: string;
...
export const ATTRIBUTE_CATALOG: Record<string, MasterAttributeDef> = {
    ...
    includeInTableView: {
        ...
        label: 'Include in Table view',
        kind: 'checkbox',
        hint: 'When on, this field may appear as a column in the records list (subject to table settings).',
    },
    includeInInlineEdit: {
        key: 'includeInInlineEdit',
        label: 'Include in Inline Edit',
        kind: 'checkbox',
        hint: 'When on, this field may be edited inline in the records grid when inline edit is enabled.',
    },
    ...
    linkClickBehavior: {
        key: 'linkClickBehavior',
        label: 'Click behavior',
        kind: 'select',
        options: [
            { value: 'new_page', label: 'Clickable - New Page' },
            { value: 'same_page', label: 'Clickable - Same Page' },
        ],
        hint: 'URL: opens in new tab vs current tab. Email: stored for consistency; links use mailto: (client default).',
    },
    currencySymbol: {
        key: 'currencySymbol',
...
/** Ordered master list: matches standard “Data Type” picker groupings. */
export const FIELD_TYPE_MASTER: FieldTypeMasterEntry[] = [
    {
        ...
        description:
            'BlockNote page document stored per record. Edited in the Notion page editor, not in the grid.',
        attributeKeys: ['description', 'helpText', 'includeInTableView', 'preferredInView'],
    },
    {
        id: 'autoNumber',
        category: 'System',
...
/** Governance + data-view flags appended to every type (Briselle field scheme). */
const EFFECTIVE_KEYS_SUFFIX: readonly string[] = [
    'piiData',
    'hiiData',
    'financialData',
    'preferredInView',
...
export function getDefaultAttributesForFieldType(type: ObjectFieldDataType): Record<string, unknown> {
    ...
    for (const key of getEffectiveAttributeKeys(type)) {
        switch (key) {
            ...
            case 'includeInTableView':
                out[key] = true;
                break;
            case 'includeInInlineEdit':
                out[key] = false;
                break;
            case 'emailVerificationStatus':
                out[key] = 'not_bounced';
                break;
            case 'linkClickBehavior':
                out[key] = 'new_page';
                break;
            default:
                break;
